...
boot <- nlsBoot(fit, niter=999)
# boot$bootCI #contains the bootstrap medians and the bootstrap 95% CI
# boot$coefboot # contains the bootstrap parameter estimates 

bootCoef <- data.frame(boot$coefboot) ## 999 Bootstrapped parameter estimates
colnames(bootCoef) <- c("p1", "p2", "p3")
...
Wilson_label <- rep("Wilson and Nieland 2001", 44)
...
VonB_All <- ggplot(data= subset(df, Label %in% c("This Study", "Wilson and Nieland 2001")), aes(x=x, y=y))+ 
...
ggsave("/Users/elizabethherdter/Desktop/R_workspace/Figures/All_VonB_fits.tiff", plot=VonB_All, width=7, height=4.5, dpi=300)

### plot without wilsons line ####	
VonB_All_basic <- ggplot(data= subset(df, Label %in% c("von Bertalanffy")), aes(x=x, y=y))+ 
		#colour=Label, group=Label))+ #group calls tells which points to connect with lines 
	 geom_line(aes(group=Label, colour= Label))+ # make line types based on the different labels- this will be our workaround because in a few stps we will specify the first label (obserseved) be a blank line (therefore a scatter plot)
	geom_point(data=subset(df, Label== "Observed"), size =1.5)+
	#scale_shape_manual(values=c(16,20,20))+
...
ggsave("/Users/elizabethherdter/Desktop/R_workspace/Figures/All_VonB_fits_minus_Wilson.pdf", plot=VonB_All_basic, width=7, height=4.5)
		
	
	
## Banana plot of Linf and K ##

Linf <- data.frame(bootCoef$p1)
colnames(Linf) <- c("Linf")


K <- data.frame(bootCoef$p2)
colnames(K) <- c("K")

banana <- data.frame(Linf, K)

bp<- ggplot(banana, aes(x=K, y=Linf))+
				 geom_point()+
				 xlab("k")+
				 ylab("L∞")+
				 theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank(), 																	panel.background=element_rect(fill='white', colour='black'),
				 ...
				 ggtitle("Plot of bootstrapped L∞ and k values")
