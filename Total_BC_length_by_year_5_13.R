...
newdata <- subset(mydata, Age.i. ==3 | Age.i.==4 | Age.i.==5 | Age.i.==6 | Age.i.==7, select=c(Year.of.Growth, Back.Calculated.Length, Age.i.))
...
library(bear)
testdf <- data.frame(summarySE(newdata, measurevar="Back.Calculated.Length", groupvars=c("Age.i.", "Year.of.Growth")))
testdf$Age.i. <- factor(testdf$Age.i.)  # need to be treated as factor levels so that ggplot makes them correctly


#http://www.cookbook-r.com/Graphs/Plotting_means_and_error_bars_(ggplot2)/
library(ggplot2)
pd <- position_dodge(.05)
test <- ggplot(testdf, aes(x=Year.of.Growth, y=Back.Calculated.Length, colour= Age.i.))+ 												   geom_errorbar(aes(ymin=Back.Calculated.Length-se, ymax=Back.Calculated.Length+se), width=.15)+ # position=pd)+
					geom_line()+
					...
					ggtitle("Back-Calculated Length Through Years")
