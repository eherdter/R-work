...
data= read.csv("BC_5_27_14.csv", header=TRUE)
...
data_wk = cbind(IncW, IncW^(.303),log(IncW+1), IncNum, data$Year.of.Increment.Formation, U_wind_median, V_wind_median, Av.Dis, CatchYr, Fish.ID)
## the IncW^.303 was added based on lines below that determine the optimal Boxcox transformation 
colnames(data_wk) = c("IncW", "IncW_trans","IncW_logtrans", "IncNum", "Yr.Inc.Form", "Uwind", "Vwind", "Av.Dis", "CatchYr", "Fish.ID")
...
Z <- cbind(U_wind_median, V_wind_median, Av.Dis, IncW)
...
library(nlme)
M.gls <- gls(IncW~ IncNum +fYr.Inc.Form +Uwind +Vwind +Av.Dis, data=data_wkdf, na.action=na.omit)
M1.lme <- lme(IncW ~ IncNum + fYr.Inc.Form+ Uwind + Vwind + Av.Dis, random= ~1|CatchYr, method= "REML", data=data_wkdf, na.action=na.omit)
M2.lme <- lme(IncW ~ IncNum +fYr.Inc.Form + Uwind + Vwind +Av.Dis, random= list(~1|Fish.ID, ~1|CatchYr), method="REML", data=data_wkdf, na.action=na.omit)
...
# but plotting residuals shows problems so lets try a gam. In this format we cannot include a random variable but we will see what shows up. 
...
mean_dis = mean(data_wkdf_new$Av.Dis)
data_wkdf_new$Av.Dis_zc <- data_wkdf_new$Av.Dis - mean_dis
...
op <- par(mfrow= c(4,2), mar=c(5,4,1,2))
MyYlab <- "residuals"
plot(x=F2, y=E, xlsb="fitted values", ylab= MyYlab)
...
Form= formula(IncW_trans~ IncNum + Yr.Inc.Form + Uwind_zc + Vwind_zc + Av.Dis_zc)
